...
- Find information related to the bartender's field and purchase products/services.
...
As the site's owner, another goal was to promote bartender products/services and earn money by including affiliate links where potential customers could buy from. All these links are located in the 'Start Mixing' area of the navigation bar.
...
> This can be achieved by clicking on the 'Categories' link located in the navigation bar and then clicking on the 'Add Category' button available on the displayed page. After filling in a form, the recipe can be saved by clicking on the 'Save' button.
...
- As a user, I want to have access to bartender products/services information. 
...
- Edit Recipe - allows users to edit any existing cocktail recipe.
...
- Links to access/purchase bartender related material: books, tools and training.
...
The website pages are displaying and working correctly on different screen sizes for Google Chrome, Firefox, Microsoft Edge. 
...
1. All pages: 
    ...
    The navigation bar and footer elements (fonts, icons, dropdown menu)are displaying correctly and all the links are redirecting the correct locations.

2. Recipes.html

    - Search Area:
    
        The search input field and the search button are working correctly and the search result is displayed according to the user's search.
...
3. AddRecipe.html and AddCategory
...
4. EditRecipe.html and EditCategory
...
Download the application repository by clicking on 'Clone or Download' button located at the top of this page. Click on ‘Download ZIP’ and extract the files in a folder.

Within the IDE, open the folder that contains the repository.

Create a .env file containing the MongoDB credentials MONGO_URI='mongodb+srv://<username>:<password>@<cluster_name>-qtxun.mongodb.net/<database_name>?retryWrites=true&w=majority'
Install the required packages by running the command `pip -r requirements.txt`

Create a new MongoDB database and name it as 'cocktails-recipes', then create the following collections:
...
Use the command `python app.py` to run the application and it will run at 'http://127.0.0.1:8080'
